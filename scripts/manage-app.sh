...
if [ $(yq ".apps | has(\"$APP\")" $CONFIG_FILE) == "false" ]; then
    echo "$APP not found in apps in config file"
    exit 1
fi

method=$(yq ".apps.\"$APP\".method" $CONFIG_FILE)
namespace=$(yq ".apps.\"$APP\".namespace | select (.!=null)" $CONFIG_FILE)
extra_args=$(yq ".apps.\"$APP\".extraArgs | select (.!=null)" $CONFIG_FILE)
